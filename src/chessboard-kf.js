...
//
// Forked from 98facdd by Cody Mims
...
; (function () {
  ...
  buildImg.src = "img/buttons/build.png";
  var snipeImg = document.createElement("img");
  snipeImg.src = "img/buttons/snipe.png";
  var cloakImg = document.createElement("img");
  cloakImg.src = "img/buttons/cloak.png";
  var stunImg = document.createElement("img");
  stunImg.src = "img/buttons/stun.png";
  ...
  function constructor(containerElOrString, config) {
    ...
    function buildSparePiecesHTML(color) {
      var pieces = ['wK', 'wQ', 'wR', 'wB', 'wN', 'wP', 'wW']
      if (color === 'black') {
        pieces = ['bK', 'bQ', 'bR', 'bB', 'bN', 'bP', 'bW']
      }
    ...
    function animateSquareToSquare(src, dest, piece, completeFn) {
      ...
      function onFinishAnimation1() {
        $('#' + squareElsIds[dest]).find('.' + CSS.piece).remove()
    ...
    function beginDraggingPiece(source, piece) {
      /* run their custom onDragStart function
      their custom onDragStart function can cancel drag start */
      if (config.onDragStart(piece) === false) {
        return
      }
      ...
      if (source !== 'spare') {
        ...
        switch (specialPiece.charAt(1)) {
          ...
          case "R":
            ...
            break;
          case "B":
            if ($('#' + squareElsIds[source]).hasClass("cloaked")) return;
            img = cloakImg;
            break;
          default:
            return;
        }
        if (specialMoves(source, specialPiece).length > 0) {
          img.className = "special";
        } else {
          img.className = "special inactive";
        }
        img.style.display = '';
        document.getElementById(squareElsIds[source]).appendChild(img);
      }
    }
    ...
    function stopDraggedPiece(location) {
      ...
      if (action === 'snapback') {
        ...
        var sourceNode = document.getElementsByClassName("square-" + draggedPieceSource);
        if (sourceNode[0].classList.contains("cloaked")) {
          var targetNode = document.getElementsByClassName("square-" + location);
          targetNode[0].classList.add("cloaked");
          dropDraggedPieceOnSquare(location);
          sourceNode[0].classList.remove("cloaked");
        } else {
          animateSquareToSquare(draggedPieceSource, location, draggedPiece);
          dropDraggedPieceOnSquare(location);
        }
      }
    }
    ...
    widget.removeYellowHighlights = function () {
      removeSquareHighlights();
    }
    ...
    widget.stopDraggedPiece = function (location) {
    ...
    function mousedownSquare(evt) {
      if (evt.target.matches('.special') && !evt.target.parentElement.matches('.cloaked')) {
        if (evt.target.matches('.inactive')) {
          stopDraggedPiece('offboard');
        } else {
          inSpecial = true;
          var specials = specialMoves(specialSquare, specialPiece);
          removeHighlights();
          $(".special").hide();
          if (specialPiece.charAt(1) == "B") {
            evt.target.parentElement.classList.add("cloaked");
            specialSquare = "offboard";
            endSpecial();
            advanceTurn();
            endTurn();
          } else {
            for (var i = 0; i < specials.length; i++) {
              highlightSquare(specials[i], "red");
            }
          }
        }
      } else {
        ...
        if (isDragging) {
          ...
          if (evt.target.matches(".inactive")) return;

          beginDraggingPiece(square, currentPosition[square])
        }
      }
    }
    ...
    function addEvents() {
      ...
      // mouse drag pieces
      $board.on('mouseup', '.' + CSS.square, mousedownSquare)
      $container.on('mouseup', '.' + CSS.sparePieces + ' .' + CSS.piece, mousedownSparePiece)
